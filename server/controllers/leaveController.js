...
export const createLeave = async (req, res) => {
    try{
        ...
        if(employee.isDeleted){
            return res.status(403).json({error: "Your account is deactivated. You cannot apploy for leave."});
        }
        ...
        if(new Date(startDate) <= today || new Date(endDate) <= today){
         ...
         if(new Date(endDate) < new Date(startDate)){
            return res.status(400).json({error: "End Date cannot be bofore start Date"});
        }
