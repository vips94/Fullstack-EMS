...
const LoginLanding = () => {
  const portalOptions = [
    ...
    },
    {
      ...
      description:
        "View your profile, track attendance, requeset time off, and access payslips",
      icon: UserIcon,
    },
  ];
        ...
        <div className="w-full max-w-md animate-fade-in relative z-10">
          ...
          <div className="mb-10 text-center md:text-left">
            ...
            <p className="text-slate-500">
              {" "}
              Select your portal to securly acess the system
            </p>
          </div>
          {/* Portals List */}
          <div className="space-y-4">
            {portalOptions.map((portal) => (
              <Link
                key={portal.to}
                to={portal.to}
                className="group block bg-slate-50 border border-slate-200 rounded-lg p-5 sm:p-6 transition-all duratoin-300 hover:border-indigo-400 hover:bg-indigo-50"
              >
                <div className="relative z-10 flex items-center justify-between gap-4 sm:gap-5">
                  ...
                  <ArrowRightIcon className="w-4 h-4 text-slate-400 group-hover:text-indigo-600 group-hover:translate-x-1 transition-all duratoin-300" />
                </div>
              </Link>
            ))}
          </div>
